...
export default async function Contact({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-6 lg:px-12 pt-28 pb-32">
      ...
      <h1 className="serif text-5xl md:text-7xl mb-8 max-w-3xl" style={{ color: "#3a170b" }}>{dict.contact.title}</h1>
      <p className="max-w-2xl leading-relaxed text-lg mb-20" style={{ color: "#714928" }}>{dict.contact.intro}</p>
        ...
        <aside className="space-y-10 lg:pl-8 lg:border-l" style={{ borderColor: "#c49e5733" }}>
          <div>
            ...
            {info.addressLines.map((l) => (
              <div key={l} className="serif text-lg" style={{ color: "#3a170b" }}>{l}</div>
            ))}
          </div>
          <div>
            <div className="eyebrow mb-3">{info.phone}</div>
            <div className="serif text-lg" style={{ color: "#3a170b" }}>{info.phoneValue}</div>
          </div>
          <div>
            <div className="eyebrow mb-3">{info.email}</div>
            <div className="serif text-lg" style={{ color: "#3a170b" }}>{info.emailValue}</div>
          </div>
          <div>
            <div className="eyebrow mb-3">{info.hours}</div>
            <div className="serif text-lg" style={{ color: "#3a170b" }}>{info.hoursValue}</div>
          </div>
        </aside>
      </div>
    </section>
