  ...
  return (
    <>
      <section className="max-w-[1400px] mx-auto px-8 lg:px-12 pt-28 pb-16">
        ...
        <h1 className="serif text-5xl md:text-7xl max-w-3xl" style={{ color: "#2a1610" }}>{dict.sell.title}</h1>
        <p className="mt-8 max-w-2xl leading-relaxed text-lg" style={{ color: "#8a7363" }}>{dict.sell.intro}</p>
      </section>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {dict.sell.steps.map((s, i) => (
            <div key={i} className="pt-8" style={{ borderTop: "1px solid #c49e5744" }}>
              ...
              <div className="serif text-2xl mb-3" style={{ color: "#2a1610" }}>{s.t}</div>
              <p className="leading-relaxed" style={{ color: "#8a7363" }}>{s.d}</p>
            </div>
          ))}
        </div>
      </section>
