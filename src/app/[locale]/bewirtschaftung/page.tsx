  ...
  return (
    <>
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 pt-28 pb-16">
        ...
        <h1 className="serif text-5xl md:text-7xl max-w-3xl" style={{ color: "#3a170b" }}>{dict.manage.title}</h1>
        <p className="mt-8 max-w-2xl leading-relaxed text-lg" style={{ color: "#714928" }}>{dict.manage.intro}</p>
      </section>
        ...
        <ul className="grid md:grid-cols-2 gap-x-12 gap-y-2">
          {dict.manage.services.map((s, i) => (
            <li key={i} className="flex items-baseline gap-6 py-5" style={{ borderTop: "1px solid #c49e5733" }}>
              ...
              <span className="serif text-xl" style={{ color: "#3a170b" }}>{s}</span>
            </li>
          ))}
        </ul>
      </section>
